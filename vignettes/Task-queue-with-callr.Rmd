...
if all pushed tasks are still running. It retuns `NULL` if no task has
...
last two is somewhat technical. A task is _ready_ if the remote R session
has finished running it. The queue hasn't read out its result yet, and
the R session is still assigned to it. (I.e. the task's `worker` column is
not `NULL`.) A task is _done_ if the queue has already read out the result
of the function call, and has reassigned the R session to another task,
so its `worker` column is `NULL`.
...
their remote R process has started up. After this initial _running_ state
the idle tasks are always either _waiting_ or _ready_.
...
`callr::r_session$new()` starts a remote R process. The `wait = FALSE`
...
tasks on the selected remote R workers, and it also reads out the results
after they are done. I.e. it performs the _waiting_ to _running_ and
...
task cannot change, even if the remote R session itself has finished.
...
signals if the R session has finished with the computation (or it errored
while working on it). `processx::poll()` returns a list of character
vectors, one entry for each (_running_) task. This is `"ready"` if the
session is ready with the task. (Or it is `"silent"` if it is not ready,
or `"timeout"` if the time limit expired and no workers are ready.)
...
new compuration on the workers.
...
3. Calls `fun(args)` in the remote session.
...
* `result`: the R object returned from the function. This is `NULL` on
  error.
* `stdout`: the standard output of the remote session, while running the
  function.
...
second is the original error, thrown in the remote process. To help with
debugging, the error from the remote process includes a stack trace:

```{r}
res$error$parent$trace
```

For this simple remote function that just calls `stop()` the trace is not
...
3. Make the queue interrupt-safe. All operations of the queue (e.g.
   ...
   don't always leave the task data frame and the remote sessions in a
