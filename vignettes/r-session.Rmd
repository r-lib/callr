...
By default `r_sessiob$new()` blocks, i.e. it does not return until the R
session is up and running and ready to run R commands.
If an error happens when starting the process, including an R error, then
...
To terminate an R sssion, call its `$close()` method:
...
and they will be terminated when R object that represents them is garbage
collected.
...
You can use `processx::poll()` to wait for the R sesion being ready, with a
timeout.
...
starting up.
It also lets you start up multiple processes concurently, see the next
section.
...
By default these will be `"silent"` because the standard output and error
is ignored.
This can be changed via the `options` argument and `r_session_options()`.

Once `poll()` reports a `"ready"` `process` connection, you can call the
...
Note, however, that if the process may generate enough output on stderr
...
## Running multipe R sessions

If you need to start several R sessions quickly, then it is best to use
`wait = FALSE` and then `poll()` for all processes until they are all
ready.
...
Then you can either use the `$read()` method to read out the result
...
If the `callr.traceback` is `TRUE`, then callr saves the full trace,
including the the frames.
You can then inspect these frames with the `$debug()` method.
We can use it to debug the previous error:
...
It is best to do this when the subprocess is idle.
...
properly, but nevertheless it can be already useful.
